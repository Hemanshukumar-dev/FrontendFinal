          ...
          try {
              const response = await fetch(`${API_URL}?key=${API_KEY}`, {
                  ...
                  body: JSON.stringify({
                      contents: [{
                          parts: [{
                              text: `You are a career counselor AI assistant. Please provide concise career guidance and advice in response to this question: ${userMessage}. Format your response as follows:

1. Brief introduction (1-2 sentences)
2. Key points (maximum 3-4 points)
3. Short explanation for each point (1-2 sentences)
...
- Limit total response to 150-200 words
...
Conclusion sentence.`
              ...
              // Add AI response
              const aiResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || "⚠️ Couldn't process query. Please try again.";
              ...
              addAIMessage('⚠️ Error processing request. Please try again.');
          }
      }
